...
while True:
    ...
    print()
    print("CoinMarketCap Exlorer Menu")
    ...
    ticker_url = 'https://api.coinmarketcap.com/v2/ticker/?strcture=array&sort=' 
    ...
    if hour_change is not None:
        if hour_change>0:
            hour_change = Back.GREEN +str(hour_change) + '%' +styl.RESET_ALL
        else:
            hour_change = Back.RED +str(hour_change) + '%' +styl.RESET_ALL
    if day_change is not None:
        if day_change>0:
            day_change = Back.GREEN +str(day_change) + '%' +styl.RESET_ALL
        else:
            day_change = Back.RED +str(day_change) + '%' +styl.RESET_ALL
    if week_change is not None:
        if week_change>0:
            week_change = Back.GREEN +str(week_change) + '%' +styl.RESET_ALL
        else:
            week_change = Back.RED +str(week_change) + '%' +styl.RESET_ALL
